...
@onready var status_label: Label = get_node_or_null("CanvasLayer/Panel/VBox/Status")
@onready var details_label: Label = get_node_or_null("CanvasLayer/Panel/VBox/Details")
@onready var reset_button: Button = get_node_or_null("CanvasLayer/Panel/VBox/Buttons/ResetAnchor")
@onready var toggle_button: Button = get_node_or_null("CanvasLayer/Panel/VBox/Buttons/ToggleModels")

var _state: DemoState = DemoState.SEARCHING
var _lost_since_sec := -1.0
var _models_visible := true
...
func _ready() -> void:
	...
	if reset_button:
		reset_button.pressed.connect(_on_reset_anchor_pressed)
	if toggle_button:
		toggle_button.pressed.connect(_on_toggle_models_pressed)
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed and not event.echo:
		if event.keycode == KEY_R:
			_on_reset_anchor_pressed()
		elif event.keycode == KEY_H:
			_on_toggle_models_pressed()
...
func _on_marker_anchor_added(_anchor: XRAnchor3D, _tracker_name: StringName) -> void:
	...
	qr_manager.set_models_visible(_models_visible)
...
func _on_reset_anchor_pressed() -> void:
	qr_manager.reset_marker_anchors()
	_lost_since_sec = -1.0
	_set_state(DemoState.SEARCHING, "Anchor reset. Scan QR again")

func _on_toggle_models_pressed() -> void:
	_models_visible = not _models_visible
	qr_manager.set_models_visible(_models_visible)
	if toggle_button:
		toggle_button.text = "Hide Models" if _models_visible else "Show Models"

func _set_state(new_state: DemoState, detail: String = "") -> void:
	_state = new_state
	match _state:
		DemoState.SEARCHING:
			if status_label:
				status_label.text = "State: SEARCHING"
		DemoState.DETECTED:
			if status_label:
				status_label.text = "State: DETECTED"
		DemoState.ANCHORED:
			if status_label:
				status_label.text = "State: ANCHORED"
		DemoState.TRACKING_LOST:
			if status_label:
				status_label.text = "State: TRACKING LOST"
		DemoState.UNSUPPORTED:
			if status_label:
				status_label.text = "State: UNSUPPORTED"
	if detail != "" and details_label:
		details_label.text = detail
